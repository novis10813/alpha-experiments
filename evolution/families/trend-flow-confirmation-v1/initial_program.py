...
# EVOLVE-BLOCK-START
    def __init__(self, config: EvolutionStrategyConfig) -> None:
        ...
        self.closes: deque[float] = deque(maxlen=60)
    ...
    def on_data(self, data: CustomData) -> None:
        ...
        broad_up = state.close > mean(history[-20:]) and state.close > history[-20]
        flow_agrees = state.trade_imbalance > 0.0 and state.volume_imbalance > 0.0
        book_agrees = state.depth10_obi_mean > 0.0 and state.depth10_obi_last > 0.0
        entry_state = broad_up and flow_agrees and book_agrees
        exit_state = not broad_up or not (flow_agrees and book_agrees)
