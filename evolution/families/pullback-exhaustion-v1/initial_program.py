...
# EVOLVE-BLOCK-START
    ...
    def on_data(self, data: CustomData) -> None:
        ...
        pullback = state.close < previous and state.trade_imbalance < 0.0
        recovering = state.close >= previous and state.trade_imbalance >= 0.0
        if pullback:
            self.pullback_bars += 1
        elif not recovering:
            self.pullback_bars = 0
        entry_state = broad_up and 1 <= self.pullback_bars <= 3 and recovering
        exit_state = not broad_up or (state.trade_imbalance < 0.0 and state.depth10_obi_last < 0.0)
